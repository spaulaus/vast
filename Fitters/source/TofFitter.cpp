///@file TofFitter.cpp
///@brief A class to fit ToF spectra using RooFit and perform MC analysis of the fit
...
///This constructor performs the fit of the ToF
TofFitter::TofFitter(const FitHandler &fit, const FileHandler &fls, const CrystalBallParameters &pars) {
    cbPars_ = pars;
    fit_ = fit;
    fls_ = fls;
...
void TofFitter::CheckFileExistance(void) {
    ...
    if (test.fail())
        throw TofFitterException("I couldn't open the data file! What I got was " + dataFile_);
    test.close();
}
...
void TofFitter::PerformFit(void) {
    ...
    for (unsigned int i = 0; i < yields_.size() * 2; i++) {
        ...
        fitRes_.insert(make_pair(tmp.GetName(), make_pair(tmp.getValV(), tmp.getError())));
    }

    ofstream results(output_.c_str());
    if(results.fail())
        throw TofFitterException("TofFitter::PerformFit - Couldn't write to " + output_);
    ...
    for (unsigned int i = 0; i < yields_.size(); i++) {
        ...
        results << i << " " << mew.first << " " << mew.second << " " << y.first << " " << y.second << " "
                << cbPars_.CalcSigma(mew.first) << " " << cbPars_.CalcAlpha(mew.first) << " "
                << cbPars_.CalcN(mew.first) << " " << y.first / y.second << endl;
    }
    results.close();

    RooPlot frame("frame", "", tof, rng_.first, rng_.second, (rng_.second - rng_.first) * binning_);
    frame.SetTitle("");
