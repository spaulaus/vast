///@file ResponseFunctionAnalyzer.cpp
///@brief Analyzes the Response functions and convolutes them with the beta resolution
...
ResponseFunctionAnalyzer::ResponseFunctionAnalyzer(const pugi::xml_node &base) {
    ...
    if (!IoHelpers::CheckFileOrDirectoryExistance(inputDir_))
        throw VastIoException("ResponseFunctionAnalyzer::ResponseFunctionAnalyzer - We couldn't open the input "
                                      "directory : " + inputDir_);

    outputDir_ = base.child("outputBase").child_value();
    if (!IoHelpers::CheckFileOrDirectoryExistance(outputDir_))
        throw VastIoException("ResponseFunctionAnalyzer::ResponseFunctionAnalyzer - We couldn't open the output "
...
double ResponseFunctionAnalyzer::CalcBetaRes(const double &tof) {
...
void ResponseFunctionAnalyzer::FitMc(const double &en, const double &mu, const double &sigma, const double &alpha,
    ...
    if (mcFit->statusCodeHistory(0) == 0) {
        ...
        throw ResponseFunctionAnalyzerException(
                "ResponseFunctionAnalyzer::FitMc - The fit didn't converge. This is problematic...");
}

///This method calculates the fit of the simulated data.  This fit is used to know how large the neutron tails are in
/// the real data.
void ResponseFunctionAnalyzer::FitSim(void) {
    ...
    for (const auto it : energies_) {
        ...
        if (!IoHelpers::CheckFileOrDirectoryExistance(inputName))
            throw VastIoException(
                    "ResponseFunctionAnalyzer::FitSim - We couldn't open the input file : " + inputName + "!!");
        ...
        RooFitResult *fitResult = model.fitTo(*data, NumCPU(3), Save(), Range(low, high));

        if (fitResult->statusCodeHistory(0) != 0)
            throw ResponseFunctionAnalyzerException(
                    "ResponseFunctionAnalyzer::FitSim - The fit didn't converge. This is problematic...");
        ...
        simOut << setprecision(6) << it.first << " " << mu.getValV() << " "
               << sigma.getValV() << " " << sigma.getError() << " "
               << alpha.getValV() << " " << alpha.getError() << " "
               << n.getValV() << " " << n.getError() << " " << endl;
        FitMc(it.first, mu.getValV(), sigma.getValV(), alpha.getValV(), n.getValV(), convOut);
    }//for(const auto &it : energies_)
